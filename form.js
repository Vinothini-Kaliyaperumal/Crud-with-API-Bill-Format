...
function handleSubmit(event) {
    ...
    if (!purchaseDate) {
        document.getElementById('purchase-date').classList.add('is-invalid');
        valid = false;
    } else {
        document.getElementById('purchase-date').classList.remove('is-invalid');
    }
    ...
    productRows.forEach(row => {
        ...
        if (!productName) {
            row.querySelector('select[name="productName"]').classList.add('is-invalid');
            valid = false;
        } else {
            row.querySelector('select[name="productName"]').classList.remove('is-invalid');
        }

        if (!quantity || quantity <= 0) {
            row.querySelector('input[name="quantity"]').classList.add('is-invalid');
            valid = false;
        } else {
            row.querySelector('input[name="quantity"]').classList.remove('is-invalid');
        }
